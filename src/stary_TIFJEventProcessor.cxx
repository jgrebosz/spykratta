...

{
    ...
    ltb_kratta.ReadFile("./calibration/kratta.ltb");
...
#define NEW_STRUCTURE_OF_SUBEVENT_WITH_2_ADDITONAL_WORDS false // true

#if NEW_STRUCTURE_OF_SUBEVENT_WITH_2_ADDITONAL_WORDS
    //    cout <<"Length of karatta subevent " << p_se[0]
    //        << "\n   Event number subeventu kratty " << p_se[1]
    //        << endl;
    int kratta_block_size   = p_se[0] -2;
    p_se += 2;
    length -= 2 ;
    //    cout << " length = " << length << endl;
    cout << " kratta_block_size = " << kratta_block_size << endl;
        ...
        //for(int nr_visiting = 0 ; nr_visiting < 1; ++nr_visiting)
    {
        ...
        visitcard * visit_ptr = ( visitcard *)(p_se + nr_of_analysed_words);

        uint32_t how_many_words = visit_ptr->size;

        //                cout << "Info from visitcard, Type = " << visit_ptr->type << endl;
...
#define NOWY_PIOTR 1
        ...
        {
            cout << "Wykryta wizytowka digitizera V1724" << endl;
...
*/
            ...
            for(int ch = 0 ; ch < d1724.GetNchan() ; ++ch)
            {
                ...
                for(int s = 0 ; s < 3; ++s)   // s is  0 = time, 1 -> amplitude  3-> pedestal
                {
...
/*                       // it must be kratta, because only this data we loaded into LUT
                       ...
                       cout << "LOOKUP_PIOTRA: "
                            ...
                            << endl;
*/
                       int to_which = (3*s + pd_kratty);

                            //if(mod == 0)
//                            {
//                                cout << "Kratta_" << id_kratty << ", signal type (ampl/ped/time) s = "
//                                     << s
//                                 << ", value = " << wart << ", to_which matrix element= " << to_which
//                                 << endl;
//                            }
            ...
            //continue;  //   <---does it make sense?

            //...
        }  // end if(visit_ptr->type == string("V1724"))
        else {

        cout << "Unexpected visiting card here " << __func__
             << " line:" << __LINE__ << endl;
        ...
        // cout << "TSTEING B linia " << __LINE__ << endl;

        p_se += skip_vc_in_words   ;
        length -= skip_vc_in_words;

        int addr_mod = 0;
        //    while (addr_mod < kratta_block_size - 1)
        while (addr_mod < length- 1)        // czy to -1 ma sens
            //do
        {
            ...
            if(1 || false) // just to avoid warning about the unused variables from above
            {
                cout  << "\nNowa grupka danych  "
                      << "i_1010 = " <<  (int) i_1010
                      << "  i_EVENT_SIZE = "
...
//                cout << "i_TRIG_TIME_TAG = " <<  i_TRIG_TIME_TAG << endl;
            ...
            {
                static int licznik;
                cout << ++licznik << " Kratta event corrupted ? (there is no '0xa'' word) [B] note: i_EVENT_SIZE=" << i_EVENT_SIZE << endl;
                break ; //return;
            }

            if(i_EVENT_SIZE <= 0 ||  i_EVENT_SIZE > 20000) {
                cout << "Event size " << i_EVENT_SIZE<<  " is to big (probably it is corrupted). Skipping it." << endl;

                //                cout  << "MORE INFO about this event:\n"
                //                      << "i_1010 = " <<  (int) i_1010
                //                      << "  i_EVENT_SIZE = "
                //                      << i_EVENT_SIZE
                //                      << " ze slowa danych " << hex << p_se[addr_mod+0] << dec
            ...
            for(int ich = 0; ich < 8; ich++) {
                ...
                if(i_CHANNEL_MASK & (1 << ich)) { //fired channel
                    ...
                    if(t.detector_id  == -1) {
                        ...
                        //if( ! (++licznik % 5000) )
                        {
                            cerr << "Blad lookup table dla kratta - kombinacji "
                                 << i_BOARD_ID << " channel " << ich
                                 << " nie przewidziano w lookup table  calibration/geo_map.geo"
                                 << " zdarzenie ignorowane " <<  endl;
                            return;

                            // exit(0);
                        }
                    }
                    int mod  =  t.detector_id ; // <---Fake   // gg->GetModuleNo(ggi);   // module number
                    int isig =  t.subdetector;  //  <--- Fake   //gg->GetSignalNo(ggi);   // signal nr  (kazdy modul daje 3 rodzaje sygnalow)
                    bool flag_this_is_plastic = t.flag_plastic;
                    ...
                    cout << dec << "wg Lookup table: "
                         << "  i_BOARD_ID = " << setw(2) << i_BOARD_ID
                         << ", ich " << ich
                         << " ---> module " << setw(2) << mod
                         << ", isignal" << setw(2) << isig
                         << ", flagOK = "<< flag_ok << ",   label = " << label
                         << endl;



                    if(mod < 0 || mod >= KRATTA_NR_OF_CRYSTALS) {
                        cout << "Wrong module nr = " << mod << " >= " << KRATTA_NR_OF_CRYSTALS << " given by kratta lookup table\n" << endl;
                        continue;
                    }
                    ...
                    bool flag_negative_polarity_correction = false;

                    vector<int32_t> tabliczka;
                    ...
                    for(int is = 0; is < n_sample / 2; is++) {
                        ...
                        if( is == 0 && i_data_lsb > 8192){
                            flag_negative_polarity_correction = true;
                            // Sometimes some signals from a particular detector
                            // are send in a negative polarity. Piotr Pawłowski says,
                            // that in such a case - test the first posiible value.
                            // If it is > 8192 that means is is really negative.
                            // To convert is into positive, make for all data:   value=16838-value );

                        }

                        if(flag_negative_polarity_correction)
                        {
                            i_data_lsb = 0x3fff - i_data_lsb;
                        }
                        ...
                        //                        if(is < 10) cout << "Bhist dla mod " << mod << ", sig " << isig
                        //                         // 			  << ", kanal bin_nr " << bin_nr
                        //                                      << "  [" << is << "] = "
                        //                                      << " setBinContent z wartoscia " << i_data_msb
                        //                                      << endl; // hraw[mod][isig]->SetBinContent(bin_nr,float(i_data_msb));

                        if(flag_negative_polarity_correction)
                        {
                            // cout << i_data_msb ;
                            i_data_msb = 0x3fff - i_data_msb;
                            //                        cout << " po korekcji negative polarity -> " << i_data_msb << endl;
                        }
...
#define WERSJA_PIOTRA false
#if WERSJA_PIOTRA

                    cout << "TSTEING WERSJA_PIOTRA , linia " << __LINE__ << endl;

                    TSignal_processing::Signal   results ;
                    //                        {
                    //                        public:
                    ...
                    if( TSignal_processing::signal_processing(
                                tabliczka.data(), 		// pointer to the data buffer
                                tabliczka.size(), 		// Number of samples to read,
                                &results 	// output
                                ) )
                    {
                        //cout << "Success in processing the time = " << results.Tstart << endl;
                        flag_ok = true;
                    }else{
                        //cout << "Failure  in processing the time";
                        flag_ok = false;
                    }
                    time = results.Tstart;
                    pds = results.pedestal;





#else // wersja moja zamiast tej powyzszej Piotrowej

                    // cout << "TSTEING D linia " << __LINE__ << endl;

                    if(ch_max == 0 || (ch_max > ( tabliczka.size() - 10))) {
                        // no maximum, or just near the end of the graph - we ignore it
                        flag_ok = false;
                    }
...
#define DISK if(0)
                    ...
                    DISK plik.write((char *) &dlug, sizeof(dlug));

                    for(unsigned int is = 0; is < tabliczka.size(); is++) {
                        tabliczka[is] -= pds;

                        DISK plik.write((char *) &tabliczka[is], sizeof(tabliczka[is]));
                    ...
                    // sa tu zmienne ktorych nazwy nie sa juz aktualne cout << "i_nevent= " << i_nevent
                    // << "  ich= " << ich << " pedestal pds= " << pds  << endl;
                    ...
                    if(1 || false)
                    {
                        cout << " For " << (flag_this_is_plastic? "Plastic" : "Kratta")
                             << " module nr " <<  mod << ", sygnal = " << isig
                             << " label: " << label
                             << "  value_max is " << value_max
                             << ", pedestal " << pds
                             << " so (max - pedestal) = " << (value_max - pds)
                             << ", time is " << time
                             << endl;
                    }


                    if(flag_this_is_plastic)
                    {

                        // Note2: this is just this part which is comimng though the digitizer
                        //        the rest is coming though the V775
                        if(mod >= KRATTA_NR_OF_PLASTICS) cerr << "!Alarm! Wrong adressing of plastic event data";

                        cout << "Plastic data. mod " << mod << ", isig = " << isig << endl;
                        target_event->plastic[mod][isig] = value_max - pds ; // normal values 0-2
                        target_event->plastic[mod][3 + isig] = pds;		// pedestals are 3-5
                        target_event->plastic[mod][6 + isig]  = time;
                    }
                    else // normal kratta
                    {
                        cout << "Kratta data. mod " << mod << ", isig = " << isig
                             << ", val_max-pds = " << (value_max - pds)
                             << ", pds = " << pds
                             << ", time = " << time
                             << endl;
                    ...
                    //cout << "Channel did not fired " << endl;
                }
            ...
            //                cout << "Na dole petli while: if (addr_mod  " << addr_mod
            //                     << " < length = " << length
            //                     << " ) (po zwiekszeniu o " << i_EVENT_SIZE << ")" << endl;
        } // end of while(addr_mod < (kratta_block_size - 1));

        // cout << "Finished Kratta part, now  addr_mod=  " << addr_mod << endl;

        // Now we can check if there is hector/phoswich data block
        //    if(length > addr_mod +3) // perhaps phoswich block is empty
        //    {
        //        unpack_hector(p_se +addr_mod, false, length);  // false means Foswitch data (not hector)
        //    }
        //    else{
        //        //cout << "No phoswich block in this event"  << endl;
        //    }
...
#define VISITING_CARD 1    // 0 - no, old style;  1 - przskok, moje rozkowodanie, 2 - pełny sposób Piotra (nie dziala?)
    ...
    for(int nr_visiting = 0 ; nr_of_analysed_words < length; ++nr_visiting)
    {
        ...
        visitcard * visit_ptr = ( visitcard *)(data_block + nr_of_analysed_words);
        uint32_t how_many_words = visit_ptr->size;
        ...
        if(visit_ptr->type == string("V1730B"))   // digitizer dla non-kratta signals
        {
            ...
            int nr_dig0_or_dig1 = 0;
            switch(visit_ptr->name[3])        //  "dig0" OR "dig1" ?
            {
            case '0':nr_dig0_or_dig1 = 0;
                break;
            case '1':nr_dig0_or_dig1 = 1;
                break;

            default:
                cout << "error in title name of visiting card ["
                     << visit_ptr->name
                     << "], Should be dig0 or dig1" << endl;
                exit(100);
            ...
            for(int n = 0 ; n < d.GetNchan() ; ++n)
            {
                if(d.Empty(n))
                    continue;
                // Piotr says we need 3 first data (signals time, slow, fast)
                for(int s = 0 ; s < 3; ++s)
                {
                    ...
                    if(wart != 0){
                        ...
                        int where = (s * 32)    // which signal (time, slow, fast)
                                + (nr_dig0_or_dig1 * 16) // form which digitizer
                                + n;  // which channel (0-16)
                        ...
                        //                      cout << "Channel " <<  ((nr_dig0_or_dig1 * 16) + n)
                        //                          << ", signal " << s << " ==> value "
                        //                              << wart << "  po korekcji = "
        ...
        else if(visit_ptr->type == string("V775"))
        {
            ...
            for(int n = 0 ; n < d775.GetNchan() ; ++n)
            {
                ...
                if(wart > 0)
                {
                    ...
                    // WARNING: from now on, the GEO IS NOT USED anymore
                    // we should recognise it with a name
                    //  visit_ptr->name
                    ...
                    // Now we idenify it with a name (not with a GEO number)

                    if(strcmp(visit_ptr->name, "tdc0") == 0)
                    {
                        target_event->hector_tdc[n] = wart;
                        cout << "v775 TDC Kanal " << n << " wartosc " << wart << " goes to hector TDC " << endl;
                        kwartet t  = lookup_kratta.current_combination(0, n);

                        // target_event->plastic[[n]
                    }
                    else if(strcmp(visit_ptr->name, "tdc1") == 0)
                    {
                        target_event->hector_tdc[n+32] = wart;
                        cout << "v775 TDC Kanal " << n+32 << " wartosc " << wart
                             << " goes to hector TDC " << endl;

                    }
                    else if(strcmp(visit_ptr->name, "adc0") == 0)
                    {
                        target_event->hector_adc[n] = wart;
                        cout << "v775 ADC Kanal " << n << " wartosc " << wart << " goes to hector ADC " << endl;
                    }

                    //                    // or it is a phoswich data ...
                    //                    else if(geo == PHOSWITCH_ADC_GEO) {// !!! This should be changed soon
                    //                        target_event->phoswich_adc[n] = wart;
                    //                    }
                    //                    else if(geo == PHOSWITCH_TDC_GEO) {// !!! This should be changed soon
                    //                        target_event->phoswich_tdc[n] = wart;
                    //                    }
                    else
                    {
                        cout << "775 ADC Unrecognised destination for n=" << n << " value = " << wart << endl;
                    }
                }
            }
            nr_of_analysed_words += how_many_words;
        ...
        {
            //cout << "Wykryty V879, rozpakowuje jako 878" << endl;
            ...
            for(int n = 0 ; n < d879.GetNchan() ; ++n)
            {
                ...
                if(wart > 0){
                    cout << "878 Kanal " << n << " wartosc " << wart << endl;
                    // distributing to the event
                    int geo = d879.geo;

                    if(geo == HECTOR_TDC_GEO) {
                        target_event->hector_tdc[n] = wart;

                    } else if(geo == HECTOR_ADC_GEO) {
                        target_event->hector_adc[n] = wart;
                    }

                    // Perhaphs it is a phoswich data ...
                    else if(geo == PHOSWITCH_ADC_GEO) {
                        target_event->phoswich_adc[n] = wart;
                    }
                    else if(geo == PHOSWITCH_TDC_GEO) {
                        target_event->phoswich_tdc[n] = wart;
                    }
        ...
        uint32_t *ptr = &visit_ptr->data;
        general_vme_word word;

        nr_of_analysed_words+= skip_vc_in_words;

        for(unsigned int i = 0; i < how_many_words - skip_vc_in_words; ++i)  // 1 bec. we skip counter how many words
        {

            cout << "Unpacking word " << i << ": " << hex
                 << (ptr[i]) << ", at adress " << &ptr[i]
                    << dec << endl;

            ++nr_of_analysed_words;
            word.word775.raw_word = ptr[i];
            ...
            case 0:
                // sizes are hardcoded in the class definition
                if(/*word.word775.data_word.geo < 32 && */  word.word775.data_word.channel < 32) {

                    //if(real_hector == false)
                    cout << "Data word recognized as geo = " << word.word775.data_word.geo
                         << ", chan = " << word.word775.data_word.channel
                         << ", data = " << word.word775.data_word.data
                         << endl;
                    if(word.word775.data_word.geo == 21 /*&& word.word775.data_word.channel == 17*/ )
                    {
                        cout << "JEST " << endl;
                    }
                    ...
                    // HECTOR_TDC_GEO = 18;  <--- now this comes from disk (hector calibr file)
                    // HECTOR_ADC_GEO =	19;<--- now this comes from disk (hector calibr file)
                    ...
                    // Perhaphs it is a phoswich data ...
                    else if(word.word775.data_word.geo == PHOSWITCH_ADC_GEO) {
                        target_event->phoswich_adc[word.word775.data_word.channel] = word.word775.data_word.data;

                        // FAKE !!!!!!!!!!!
                        //  target_event->phoswich_adc[word.word775.data_word.channel] = 100* word.word775.data_word.channel;
                    }
                    else if(word.word775.data_word.geo == PHOSWITCH_TDC_GEO) {
                        target_event->phoswich_tdc[word.word775.data_word.channel] = word.word775.data_word.data;

                        // FAKE
                        //  target_event->phoswich_tdc[word.word775.data_word.channel] = 200 *word.word775.data_word.channel;
            ...
            case 4:
                //                cout << "Footer when word nr i = " << i << endl;

                break;
            case 6:
                cout << "'Not valid datum for this TDC when i = .." << i << endl;
                break;

            default :
                cout << "Error - while unpacking "
                     << (real_hector? "Hector" : "Phoswich")
                     << " VME word nr i = " << i
                     << ". Unknown 'code' bitfield: ("
                     << word.word775.header_word.code
                     << ") skipping... " << endl;
                return;
                break;
            } // endof inner switch
...
#else
    // Visiting card Piotra
    // TO NIE DZIALA, STATYCZNY OBIEKT DataDecoder::
    // JEST TERAZ PUSTY. (Nie wiem dlaczego).

    if(DataDecoder::Size() == 0)
    {
        cout << "Na razie pusty wektor bazy dekoderow" << endl;
        DataDecoder::decoder_dbase( { "V785", deco785 } );
    }
    ...
    visitcard * vc = (visitcard *) data;
    using word_t = uint32_t;

    word_t * ptr = data;
    word_t * ptr_stop = data + length;
    DataDecoder * deco;

    while(ptr < ptr_stop){ // loop over VME cards
        vc = (visitcard*)ptr;
        ShowVisitcard(vc);
        deco = DataDecoder::Get(vc->type);
        if(deco){
            deco->Load(&vc->data);
            int nchan = deco->GetNchan(); // Number of channels
            int ndata = deco->GetDataSize();// Size of data per channel
            if(ndata == 1)
                for(int i=0; i<nchan; i++){ // loop over channels

                    //TString name = Form("%s_%d",vc->name,i);
                    //TH1F * h = (TH1F *)gROOT->FindObject(name);
                    // if(!h) {
                    //                            float xmin = deco->GetXmin();
                    //                            float xmax = deco->GetXmax();
                    //                            float nbin = deco->GetNbin();
                    //                            h = new TH1F(name, name, nbin, xmin, xmax);
                    //                        }
                    //                        h->Fill(deco->GetData(i,0));

                    cout << "A Nazwa " << vc->name << ", i = " << i << endl;
                    cout << "B Data for channel = " << i << ", dana " << deco->GetData(i,0) << endl;
                }
        }
        else
            cout << "No decoder for type = " << vc->type << endl;

        ptr += vc->size; // jump to the next board
    } // End subevent
    ...
    // cout << "end of unpacking hector" << endl;
}
//*****************************************************************************

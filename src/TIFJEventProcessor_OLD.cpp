...

{
    ...
    ltb_kratta.ReadFile("./calibration/kratta.ltb"); // lookup table
    ltb_paris.ReadFile("./calibration/paris.ltb"); // lookup table
    //ltb_plastic.ReadFile("./calibration/paris.ltb"); // lookup table

    ltb_plastic.ReadFile("./calibration/plastic.ltb"); // lookup table
    ltb_plastic.ReadFile("./calibration/silicon.ltb"); // lookup table
                ...
                for(int ch = 0 ; ch < d1724.GetNchan() ; ++ch)
                {
                    ...
                    for(int s = 0 ; s < 3; ++s)   // s is  0 = time, 1 -> amplitude  2-> pedestal
                    {
                        ...
                        if(wart != 0){
                            ...
                            {
                                current_board.Print() ;
                                cout << "ERROR: TOO BIG INDEX ch = " << ch
...
*/
        ...
        else if (visit_ptr->type == string("RATE"))
        {
            ...
            cout << "Unexpected visiting card "
                 << visit_ptr->type << ", origin: " << visit_ptr->name  << "  (here: " << __func__
                 << " line:" << __LINE__ << ")"  << endl;
...
#define PARIS_NORMALY__OTHERWISE_PLASTIC true
            ...
            if(current_board.Size() == 0)
            {
                cout << "For signals coming from digitizer  "
                     <<  visit_ptr->name
                      << " - the the lookup table has no entries"
                      << endl;
                //continue;
            } else
                for(int n = 0 ; n < d.GetNchan() ; ++n)
                {
                    ...
                    {
                        if( (++licznik % 5) == 0 )
                            cout << "For v775, " << visit_ptr->name << ", channel "
                                 << n
                                 << " is "<< (branch.Size() )
                    ...
                    for(int s = 0 ; s < 3; ++s) // we need 3 first data signals (time, fast(short), slow(long)
                    {
                        ...
                        if(wart != 0)
                        {
                            ...
                            int dig_ch = (nr_dig0_or_dig1 *16*3) + (n * 3) + s ;
            ...
            for(int n = 0 ; n < d775.GetNchan() ; ++n)
            {
                ...
                if(wart > 0)
                {
                    ...
                    else if(branch_name == string("silicon"))
                    {
                        ...
                        else if(variable_name == "time30")co = 2;

                        target_event->silicon[id][co] = wart;
                        //                        cout << "to silicon[" <<id << "] goes <--- " << wart << endl;
                    }
            ...
            for(int n = 0 ; n < d879.GetNchan() ; ++n)
            {
                ...
                if(wart > 0){
                    ...
                    else if(branch_name == string("silicon"))
                    {
                        ...
                        target_event->silicon[id][co] = wart;
                        //cout << "to silicon[" <<id << "][amplitude] goes <--- " << wart << endl;
                    }
        ...
        cout << "ARE WE HERE SOMETUIMES? " << endl;

...
int Tnamed_pointer::is_in_AND_gate(double min_value, double max_value, bool obligatory)
{
	...
	if(!flag_multi_entry)
	{
		if(data_usable != nullptr)   // adres of flag was given
		{
	...
	else
	{
		...
		{
			if(vek[m].v_data_usable != nullptr)   // adres of flag was given
			{
			...
			switch(vek[m].v_what_type)
			{
				case    Tnamed_pointer::is_double:
					value2 = (*(vek[m].v_double_ptr));
					break;
				case    Tnamed_pointer::is_int:
					value2 = (*(vek[m].v_int_ptr));
					break;
				case    Tnamed_pointer::is_bool:
					value2 = (*(vek[m].v_bool_ptr));
					break;
			}
...
int Tnamed_pointer::is_in_OR_gate(double min_value, double max_value, bool obligatory)
{
	...
	if(!flag_multi_entry)
	{
		if(data_usable != nullptr)   // adres of legality flag was given
		{
			if(*data_usable == false)  // non legal now ?
			{
				if(obligatory)
	...
	else
	{
		for(unsigned m = 0 ; m < vek.size() ; m++)
		{
			...
			{
				case    Tnamed_pointer::is_double:
					value3 = (*(vek[m].v_double_ptr));
					break;
				case    Tnamed_pointer::is_int:
					value3 = (*(vek[m].v_int_ptr));
					break;
				case    Tnamed_pointer::is_bool:
					value3 = (*(vek[m].v_bool_ptr));
					break;
			}
...
/* This function looks into the map of existing incrementers and tries
 * to find a name of an incrementer which remembers an address
 * of some variable.
 * The address of this variable we need send as an argument
 * of this funtion.
 * NOTE: This function is NOT looking into "ALL..." incrementers
 */

string Tnamed_pointer::give_incrementer_name(void *ptr)
{
	//map<string, Tnamed_pointer> named_pointer ;
	for(auto & entry : named_pointer)
	{
		if(entry.second.flag_multi_entry == true) continue;
		if(entry.second.double_ptr == ptr)
			return  entry.first;
	}
	return "";
...
/* Same as above, but this function looks only in the "ALL..." incrementers
 * In case if there are more such incrementers - it is returning the first found
 *
 */
string Tnamed_pointer::give_ALL_incrementer_name(void *ptr)
{
	for(auto & entry : named_pointer)
	{
		if(entry.second.flag_multi_entry != true) continue;
		//cout << "checking entry " << entry.first << endl;

		for(auto element : entry.second.vek)
		{
			if(element.v_double_ptr == ptr){
				// cout << "found name " << entry.first;
				return  entry.first;   // name
			}
		}
	}
	return "No iIncrementer (in the map of incrementers) uses this address . Strange...";
}
//***********************************************************************/

...
public:
    inline Vector2D polar(double rho_, double phi_)
    {
        return Vector2D(rho_ * cos(phi_), rho_ * sin(phi_));
    }
...
inline Vector3D Vector3D::polar(double rho_, double theta_, double phi_)
{
    double rho_sin_theta = rho_ * sin(theta_);
    return Vector3D(rho_sin_theta * cos(phi_), rho_sin_theta * sin(phi_), rho_ * cos(theta_));
}


namespace
{
Vector3D operator + [[ maybe_unused]](const Vector2D &a, const Vector3D &b)
{
    return Vector3D(a) + b;
}
Vector3D operator + [[ maybe_unused]](const Vector3D &a, const Vector2D &b)
{
    return a + Vector3D(b);
}
}
